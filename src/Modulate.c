...
void SendLeaderAndSYNC(UCHAR *bytEncodedBytes, int intLeaderLen) {
	...
	// note revised To accommodate 1 parity symbol per byte (10 symbols total)
			...
			for (m = 0; m < intDataBytesPerCar; m++)      // For each byte of input data
			{
				...
				for (k = 0; k < 4; k++)        // for 4 symbol values per byte of data
				{
					for (n = 0; n < intSampPerSym; n++)     // for all the samples of a symbol for 2 carriers
					{
						...
						bytSymToSend = (bytMask & bytEncodedBytes[intDataPtr + intDataBytesPerCar])
								>> (2 * (3 - k));    // Values 0-3
			...
			for (m = 0; m < intDataBytesPerCar; m++)      // For each byte of input data
			{
				...
				for (k = 0; k < 4; k++)        // for 4 symbol values per byte of data
				{
					for (n = 0; n < intSampPerSym; n++)     // for all the samples of a symbol for 2 carriers
					{
						...
						bytSymToSend = (bytMask & bytEncodedBytes[intDataPtr + 2 * intDataBytesPerCar])
								>> (2 * (3 - k));    // Values 0-3
						intSample = intSample + intFSK100bdCarTemplate[12 + bytSymToSend][n];

						// ' Fourth carrier

						bytSymToSend = (bytMask & bytEncodedBytes[intDataPtr + 3 * intDataBytesPerCar])
								>> (2 * (3 - k));    // Values 0-3
...
//	If Not (strType = "DataACK" Or strType = "DataNAK" Or strType = "IDFrame" Or strType.StartsWith("ConReq") Or strType.StartsWith("ConAck")) Then
	...
	for (m = 0; m < intDataBytesPerCar; m += 3)  // For each byte of input data
	{
		...
		for (k = 0; k < 8; k++) {
			...
			{
				if ((k & 1) == 0)
					intSample = intFSK25bdCarTemplate[bytSymToSend +
													  4][n]; // Symbol vlaues 4- 11 (surrounding 1500 Hz)
				else
					intSample = -intFSK25bdCarTemplate[bytSymToSend +
													   4][n]; // Symbol vlaues 4- 11 (surrounding 1500 Hz)
...
//	int intMask  = 7;
	...
	int3Bytes = int3Bytes << 8;
	int3Bytes += bytEncodedBytes[intDataPtr + intCar * intDataBytesPerCar +
								 2];  // now have 3 bytes, 24 bits or 8 8PSK symbols
			...
			for (k = 0; k < 4; k++)  // for 4 symbol values per byte of data
			{
				for (n = 0; n < intSampPerSym; n++)  // Sum for all the samples of a symbols 
				{
					...
					for (i = 0; i < intNumCar; i++) // across all carriers
					{
						...
						if (intBaud == 100) {
							if (bytSymToSend < 2)
								intSample += intPSK100bdCarTemplate[intCarIndex][bytSymToSend *
																				 2][n];  // double the symbol value during template lookup for 4PSK. (skips over odd PSK 8 symbols)
							else
								intSample -= intPSK100bdCarTemplate[intCarIndex][2 * (bytSymToSend -
																					  2)][n]; // subtract 2 from the symbol value before doubling and subtract value of table
						} else {
							if (bytSymToSend < 2)
								intSample += intPSK200bdCarTemplate[intCarIndex][bytSymToSend *
																				 2][n];  // double the symbol value during template lookup for 4PSK. (skips over odd PSK 8 symbols)
							else
								intSample -= intPSK200bdCarTemplate[intCarIndex][2 * (bytSymToSend -
																					  2)][n]; // subtract 2 from the symbol value before doubling and subtract value of table
						}
					...
					for (i = 0; i < intNumCar; i++) {
						...
						if (intBaud == 100) {
							if (bytSymToSend <
								...
								intSample -= intPSK100bdCarTemplate[intCarIndex][bytSymToSend -
																				 4][n]; // negative phase values,  subtract value of table
						} else {
							if (bytSymToSend <
								4) // This uses the symmetry of the symbols to reduce the table size by a factor of 2
					...
					for (i = 0; i < intNumCar; i++) // across all carriers
					{
						...
						if (bytSym < 8) {
							...
							if (bytSymToSend <
								4) // This uses the symmetry of the symbols to reduce the table size by a factor of 2
								intSample += 0.5f *
											 intPSK100bdCarTemplate[intCarIndex][bytSymToSend][n]; // positive phase values template lookup for 8PSK.
							else
								intSample -= 0.5f * intPSK100bdCarTemplate[intCarIndex][bytSymToSend -
																						4][n]; // negative phase values,  subtract value of table
						}
						//}
						if (n == intSampPerSym - 1)        // Last sample?
...
void RemodulateLastFrame() {
	...
	if (!FrameInfo(bytEncodedBytes[0], &blnOdd, &intNumCar, strMod, &intBaud, &intDataLen, &intRSLen, &bytMinQualThresh,
				   strType))
		return;
...
void sendCWID(char *strID, BOOL blnPlay) {
	...
	int intCW[] = {
		0x17, 0x1D5, 0x75D, 0x75, 0x1, 0x15D,
