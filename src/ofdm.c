	...
	if (FirstUnackedptr) {
		...
		if (bytesacked) {
			...
			WriteDebugLog(LOGDEBUG,
						  "OFDM Acked %d Blocks %d Bytes Still outstanding %d Carriers Acked %d OFDM Acked Percent %d Quality %d",
						  index, bytesacked, BytesSent, Acked, AckedPercent, Quality);
			...
			WriteDebugLog(LOGDEBUG,
						  "OFDM Nothing Acked Bytes Still outstanding %d Carriers Acked %d OFDM Acked Percent %d Quality %d",
						  BytesSent, Acked, AckedPercent, Quality);
		...
		WriteDebugLog(LOGDEBUG, "OFDM AckedPercent %d CarriersACKed %d CarriersSent %d", AckedPercent, CarriersACKed,
					  CarriersSent);
...
//			if (OFDMMode < PSK16 || CarriersSent < 43)
		...
		WriteDebugLog(LOGDEBUG,
					  "OFDM All Acked %d Bytes removed from queue. AckedPercent %d Carriers Acked %d Quality %d",
					  BytesSent, AckedPercent, Acked, Quality);
...
int EncodeOFDMData(UCHAR bytFrameType, UCHAR *bytDataToSend, int Length, unsigned char *bytEncodedBytes) {
	...
	blnFrameTypeOK = FrameInfo(bytFrameType, &blnOdd, &intNumCar, strMod, &intBaud, &intDataLen, &intRSLen,
							   &bytQualThresh, strType);
...
VOID InitDemodOFDM() {
	...
	for (i = 0; i < intNumCar; i++) {
		...
		GoertzelRealImag(intFilteredMixedSamples + 240, intCP[i], intNforGoertzel[i], dblFreqBin[i], &dblReal,
						 &dblImag);
		dblPhase = atan2f(dblImag, dblReal);
		...
		GoertzelRealImag(intFilteredMixedSamples + 480, intCP[i], intNforGoertzel[i], dblFreqBin[i], &dblReal,
						 &dblImag);
		dblPhase = atan2f(dblImag, dblReal);
		...
		GoertzelRealImag(intFilteredMixedSamples + 720, intCP[i], intNforGoertzel[i], dblFreqBin[i], &dblReal,
						 &dblImag);
		dblPhase = atan2f(dblImag, dblReal);
...
BOOL DemodOFDM() {
	...
	while (State == AcquireFrame) {
		...
		if (SymbolsLeft <= 0) {
			...
			intLastRcvdFrameQuality = UpdatePhaseConstellation(&intPhases[intNumCar / 2][0], &intMags[intNumCar / 2][0],
															   intPSKMode, FALSE, TRUE);
			...
			WriteDebugLog(LOGDEBUG, "intDataLen 1 %d", intDataLen);
...
//	int Corrections;
	...
	for (i = 0; i < intNumOfSymbols; i++) {
		GoertzelRealImag(intFilteredMixedSamples, Start + intCP[Carrier], intNforGoertzel[Carrier], dblFreqBin[Carrier],
						 &dblReal, &dblImag);
		...
		for (k = 0; k < s; k++) {
			...
			for (i = 0; i < intNumCar; i++) // across all active carriers
			{
				if (OFDMMode == PSK2) {
					...
					bytSym = (bytEncodedBytes[intDataPtr + i * intDataBytesPerCar] >> (4 * (1 - k))) & 15;
					bytSymToSend = ((bytLastSym[intCarIndex] & 7) + (bytSym & 7) &
									7); // Compute the differential phase to send
				...
				for (n = 0; n < intSampPerSym; n++) {
					if (OFDMMode == PSK2) {
						...
						if (bytSymToSend <
							2) // This uses the symmetry of the symbols to reduce the table size by a factor of 2
							OFDMSamples[p++] += intOFDMTemplate[intCarIndex][4 *
																			 bytSymToSend][n]; //  double the symbol value during template lookup for 4PSK. (skips over odd PSK 8 symbols)
						else
							OFDMSamples[p++] -= intOFDMTemplate[intCarIndex][4 * (bytSymToSend -
																				  2)][n]; // subtract 2 from the symbol value before doubling and subtract value of table
					} else if (OFDMMode == PSK16) {
						if (bytSymToSend <
							8) // This uses the symmetry of the symbols to reduce the table size by a factor of 2
							...
							OFDMSamples[p++] -= intOFDMTemplate[intCarIndex][(bytSymToSend -
																			  8)][n]; // subtract 2 from the symbol value before doubling and subtract value of table
					} else {
						...
						if (bytSymToSend <
							4)// This uses the symmetry of the symbols to reduce the table size by a factor of 2
							OFDMSamples[p++] = intOFDMTemplate[intCarIndex][2 *
																			bytSymToSend][n]; // double the symbol value during template lookup for 4PSK. (skips over odd PSK 8 symbols)
						else if (bytSymToSend < 8)
							OFDMSamples[p++] = -intOFDMTemplate[intCarIndex][2 * (bytSymToSend -
																				  4)][n]; // subtract 4 from the symbol value before doubling and subtract value of table
						else if (bytSymToSend < 12)
							OFDMSamples[p++] = dblOFDMCarRatio * intOFDMTemplate[intCarIndex][2 * (bytSymToSend -
																								   8)][n]; // subtract 4 from the symbol value before doubling and subtract value of table
						else
							OFDMSamples[p++] = -dblOFDMCarRatio * intOFDMTemplate[intCarIndex][2 * (bytSymToSend -
																									12)][n]; //  subtract 4 from the symbol value before doubling and subtract value of table
					}
				}
