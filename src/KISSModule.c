...
VOID ProcessPacketBytes(UCHAR *Buffer, int Read) {
	// Called when frame received on TCP Packet Connection. Could be for KISS or ARDOP Packet Session mode
	// Assumes that Complete KISS or Host packet will be received (pretty safe with TCP (I hope!)
...
VOID ProcessKISSBytes(UCHAR *RXBuffer, int Read) {
	// Store in cyclic buffer, counting FENDS so we know if we have a full frame in the buffer
	UCHAR c;
