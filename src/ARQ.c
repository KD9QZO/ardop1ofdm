...
const char ARQBandwidths[9][12] = {"200FORCED", "500FORCED", "1000FORCED", "2000FORCED", "200MAX", "500MAX", "1000MAX",
								   "2000MAX", "UNDEFINED"};
enum _ARQSubStates ARQState;

const char ARQSubStates[10][11] = {"None", "ISSConReq", "ISSConAck", "ISSData", "ISSId", "IRSConAck", "IRSData",
								   "IRSBreak", "IRSfromISS", "DISCArqEnd"};
...
void CalculateOptimumLeader(int intReportedReceivedLeaderMS, int intLeaderSentMS) {
	intCalcLeader = max(200, 120 + intLeaderSentMS -
							 intReportedReceivedLeaderMS);  //  This appears to work well on HF sim tests May 31, 2015
...
static UCHAR byt2000[] = {80, 80, 80, 76, 85, 75,
						  0}; //  Threshold for 8PSK 167 baud changed from 73 to 80 on rev 0.7.2.3
	...
	if (dblQuality <= dblLowTrip) //  NAK Shift down conditions
	{
		if (intFrameTypePtr > 0) {
			...
			WriteDebugLog(LOGINFO,
						  "[ARDOPprotocol.Gearshift_2]  Shift Down: dblQuality= %f Shift down from Frame type %s New Mode: %s",
						  dblQuality, strOldMode, strNewMode);
			intShiftUpDn = -1;
			intShiftDNs++;
		...
		if (intFrameTypePtr < (bytFrameTypesForBWLength - 1)) {
			...
			strNewMode[strlen(strNewMode) - 2] = 0;
			WriteDebugLog(LOGINFO,
						  "[ARDOPprotocol.Gearshift_2]  Shift Up: dblQuality= %f Shift up from Frame type %s New Mode: %s",
						  dblQuality, strOldMode, strNewMode);
			intShiftUpDn = 1;
		...
		case ISS:
			...
			if (bytDataToSendLength > 0) {
				...
				if (strcmp(strMod, "4FSK") == 0) {
					...
					Mod8FSKDataAndPlay(bytEncodedBytes[0], bytEncodedBytes, EncLen,
									   intCalcLeader);  // Modulate Data frame
...
int GetNextFrameData(int *intUpDn, UCHAR *bytFrameTypeToSend, UCHAR *strMod, BOOL blnInitialize) {
	...
	if (blnInitialize)    //' Get the array of supported frame types in order of Most robust to least robust
	{
		...
		if (DebugLog)
			WriteDebugLog(LOGDEBUG, "[ARDOPprotocol.GetNextFrameData] Initial Frame Type: %s",
						  Name(bytCurrentFrameType));
	...
	if (DebugLog) {
		if (strShift == 0)
			WriteDebugLog(LOGDEBUG, "[ARDOPprotocol.GetNextFrameData] No shift, Frame Type: %s",
						  Name(bytCurrentFrameType));
		else
			WriteDebugLog(LOGDEBUG, "[ARDOPprotocol.GetNextFrameData] %s, Frame Type: %s", strShift,
						  Name(bytCurrentFrameType));
	}

	FrameInfo(bytCurrentFrameType, &blnOdd, &intNumCar, strMod, &intBaud, &intDataLen, &intRSLen, &bytQualThresh,
			  strType);
		...
		case DISC:
			...
			if (blnFrameDecodedOK && intFrameType == DISCFRAME) {
				...
				WriteDebugLog(LOGDEBUG,
							  "[ARDOPprotocol.ProcessRcvdARQFrame]  DISC frame received in ProtocolState DISC, Send END with SessionID= %XX Stay in DISC state",
							  bytLastARQSessionID);
			...
			if (IsCallToMe(strCallsign, &bytPendingSessionID)) // (Handles protocol rules 1.2, 1.3)
			{
				...
				if (BusyBlock) {
					if ((blnLeaderTrippedBusy && dttLastBusyClear - dttPriorLastBusyTrip < 600)
						...
						WriteDebugLog(LOGDEBUG,
									  "[ProcessRcvdARQFrame] Con Req Blocked by BUSY!  LeaderTrippedBusy=%d, Prior Last Busy Trip=%d, Last Busy Clear=%d,  Last Leader Detect=%d",
									  blnLeaderTrippedBusy, Now - dttPriorLastBusyTrip, Now - dttLastBusyClear,
									  Now - dttLastLeaderDetect);
						...
						Mod4FSKDataAndPlay(&bytEncodedBytes[0], EncLen,
										   LeaderLength);        // only returns when all sent

						sprintf(HostCmd, "REJECTEDBUSY %s", strRemoteCallsign);
						QueueCommandToHost(HostCmd);
						sprintf(HostCmd, "STATUS ARQ CONNECTION REQUEST FROM %s REJECTED, CHANNEL BUSY.",
								strRemoteCallsign);
						QueueCommandToHost(HostCmd);
				...
				if (intReply !=
					ConRejBW)    // If not ConRejBW the bandwidth is compatible so answer with correct ConAck frame
				{
					...
					Mod4FSKDataAndPlay(&bytEncodedBytes[0], EncLen,
									   intARQDefaultDlyMs);        // only returns when all sent
				} else {
...
//			ProcessUnconnectedConReqFrame(intFrameType, bytData);  //  displays data if not connnected.  
		...
		case IRS:
			...
			if (ARQState ==
				IRSConAck)        // Process ConAck or ConReq if reply ConAck sent above in Case ProtocolState.DISC was missed by ISS
			{
				if (!blnFrameDecodedOK)
				...
				if (IsConReq(intFrameType,
							 EnableOFDM)) // Process Connect request to MyCallsign or Aux Call signs as for DISC state above (ISS must have missed initial ConACK from ProtocolState.DISC state)
				{
					if (!blnListen)
						return;
					...
					if (IsCallToMe(strCallsign, &bytPendingSessionID)) // (Handles protocol rules 1.2, 1.3)
					{
						...
						if (intReply !=
							0x2E)    // If not ConRejBW the bandwidth is compatible so answer with correct ConAck frame
						{
							...
							tmrIRSPendingTimeout =
									Now + 10000;  // Triggers a 10 second timeout before auto abort from pending

							strcpy(strRemoteCallsign, bytData);
							strcpy(strLocalCallsign, strCallsign);
							strcpy(strFinalIDCallsign, strCallsign);

							EncLen = EncodeConACKwTiming(intReply, intLeaderRcvdMs, bytPendingSessionID,
														 bytEncodedBytes);
							Mod4FSKDataAndPlay(&bytEncodedBytes[0], EncLen,
											   LeaderLength);        // only returns when all sent
						...
						sprintf(HostCmd, "REJECTEDBW %s", strRemoteCallsign);
						QueueCommandToHost(HostCmd);
						sprintf(HostCmd, "STATUS ARQ CONNECTION FROM %s REJECTED, INCOMPATIBLE BANDWIDTHS.",
								strRemoteCallsign);
						QueueCommandToHost(HostCmd);
				...
				if (intFrameType >= 0x39 && intFrameType <=
											0x3C)    // Process ConACK frames from ISS confirming Bandwidth and providing ISS's received leader info.
				{
					...
					sprintf(HostCmd, "STATUS ARQ CONNECTION FROM %s: SESSION BW = %d HZ", strRemoteCallsign,
							intSessionBW);
					QueueCommandToHost(HostCmd);
					...
					GetNextFrameData(&intShiftUpDn, 0, NULL,
									 TRUE);        //' just sets the initial data, frame type, and sets intShiftUpDn= 0
			...
			if (ARQState == IRSData || ARQState ==
									   IRSfromISS)  // Process Data or ConAck if ISS failed to receive ACK confirming bandwidth so ISS repeated ConAck
			{
				// ConAck processing from ISS
				...
				if (blnFrameDecodedOK &&
					intFrameType == DISCFRAME) //  IF DISC received from ISS Handles protocol rule 1.5
				{
					WriteDebugLog(LOGDEBUG,
								  "[ARDOPprotocol.ProcessRcvdARQFrame]  DISC frame received in ProtocolState IRS, IRSData...going to DISC state");
				...
				{
					WriteDebugLog(LOGDEBUG,
								  "[ARDOPprotocol.ProcessRcvdARQFrame]  END frame received in ProtocolState IRS, IRSData...going to DISC state");
					if (AccumulateStats) LogStats();
...
//					Mod4FSKDataAndPlay(&bytEncodedBytes[0], 16, 0);		// only returns when all sent
				...
				if (blnFrameDecodedOK && intFrameType == BREAK) {
					WriteDebugLog(LOGDEBUG,
								  "[ARDOPprotocol.ProcessRcvdARQFrame]  BREAK received in ProtocolState %s , IRSData. Sending ACK",
				...
				{
					WriteDebugLog(LOGDEBUG,
								  "[ARDOPprotocol.ProcessRcvdARQFrame]  IDLE received in ProtocolState %s substate %s",
								  ARDOPStates[ProtocolState], ARQSubStates[ARQState]);
					{
						blnEnbARQRpt = FALSE; // setup for no repeats
...
//				return;
				...
				if (blnFrameDecodedOK &&
					IsDataFrame(intFrameType)) // Frame even/odd toggling will prevent duplicates in case of missed ACK
				{
					if (intRmtLeaderMeas == 0) {
						...
						WriteDebugLog(LOGDEBUG,
									  "[ARDOPprotocol.ProcessRcvdARQFrame] IRS (receiving data) RmtLeaderMeas=%d ms",
									  intRmtLeaderMeas);
					}
					...
					if (intFrameType !=
						intLastARQDataFrameToHost) // protects against duplicates if ISS missed IRS's ACK and repeated the same frame
					{
						AddTagToDataAndSendToHost(bytData, "ARQ",
												  DataLen); // only correct data in proper squence passed to host
					...
					if (ARQState == IRSfromISS) {
						WriteDebugLog(LOGDEBUG,
									  "[ARDOPprotocol.ProcessRcvdARQFrame] Data Rcvd in ProtocolState=IRSData, Substate IRSfromISS Go to Substate IRSData");
				...
				if ((!blnFrameDecodedOK) && intFrameType == bytLastACKedDataFrameType) {
					...
					WriteDebugLog(LOGDEBUG,
								  "[ARDOPprotocol.ProcessRcvdARQFrame] Data Decode Failed but Frame Type matched last ACKed. Send ACK, data already passed to host. ");

					// handles Data frame which did not decode correctly (Failed CRC) and hasn't been acked before
				} else if ((!blnFrameDecodedOK) &&
						   IsDataFrame(intFrameType)) //Incorrectly decoded frame. Send NAK with Quality
				{
					if (ARQState == IRSfromISS) {
						WriteDebugLog(LOGDEBUG,
									  "[ARDOPprotocol.ProcessRcvdARQFrame] Data Frame Type Rcvd in ProtocolState=IRSData, Substate IRSfromISS Go to Substate IRSData");
				...
				if (intFrameType >= 0xE0) {
					...
					{
						WriteDebugLog(LOGDEBUG,
									  "[ARDOPprotocol.ProcessedRcvdARQFrame] Protocol state IDLE, ACK Received with Data to send. Go to ISS Data state.");
				...
				if (intFrameType == BREAK) {
					...
					WriteDebugLog(LOGDEBUG,
								  "[ARDOPprotocol.ProcessRcvdARQFrame] BREAK Rcvd from IDLE, Go to IRS, Substate IRSfromISS");
				...
				{
					WriteDebugLog(LOGDEBUG,
								  "[ARDOPprotocol.ProcessRcvdARQFrame]  DISC frame received Send END...go to DISC state");
...
//					Mod4FSKDataAndPlay(&bytEncodedBytes[0], 16, 0);		// only returns when all sent
		...
		case ISS:
			...
			if (ARQState == ISSConReq)  //' The ISS is sending Connect requests waiting for a ConAck from the remote IRS
			{
				...
				if (intFrameType >= 0x39 && intFrameType <=
											0x3C)  // Process ConACK frames from IRS confirming BW is compatible and providing received leader info.
				{
					UCHAR bytDummy = 0;
					...
					GetNextFrameData(&intShiftUpDn, &bytDummy, NULL,
									 TRUE);    // just sets the initial data frame type and sets intShiftUpDn = 0
					...
					WriteDebugLog(LOGDEBUG,
								  "[ARDOPprotocol.ProcessRcvdARQFrame] Compatible bandwidth received from IRS ConAck: %d Hz",
								  intSessionBW);
					ARQState = ISSConAck;
					dttLastFECIDSent = Now;
				...
				{
					WriteDebugLog(LOGDEBUG,
								  "[ARDOPprotocol.ProcessRcvdARQFrame] ConRejBusy received from %s ABORT Connect Request",
								  strRemoteCallsign);
					sprintf(HostCmd, "REJECTEDBUSY %s", strRemoteCallsign);
					QueueCommandToHost(HostCmd);
				...
				{
					WriteDebugLog(LOGDEBUG,
								  "[ARDOPprotocol.ProcessRcvdARQFrame] ConRejBW received from %s ABORT Connect Request",
								  strRemoteCallsign);
					sprintf(HostCmd, "REJECTEDBW %s", strRemoteCallsign);
					QueueCommandToHost(HostCmd);
			...
			if (ARQState == ISSConAck) {
				if (blnFrameDecodedOK && intFrameType >= 0xE0 ||
					intFrameType == BREAK)  // if ACK received then IRS correctly received the ISS ConACK
				{
					...
					if (intRmtLeaderMeas == 0) {
						intRmtLeaderMeas = intRmtLeaderMeasure; // capture the leader timing of the first ACK from IRS, use this value to help compute repeat interval.
						WriteDebugLog(LOGDEBUG, "[ARDOPprotocol.ProcessRcvdARQFrame] ISS RmtLeaderMeas=%d ms",
									  intRmtLeaderMeas);
					}
					intAvgQuality = 0;        // initialize avg quality
					...
					if (intFrameType >= 0xE0 && DebugLog)
						WriteDebugLog(LOGDEBUG, "[ARDOPprotocol.ProcessRcvdARQFrame] ACK received in ARQState %s ",
									  ARQSubStates[ARQState]);

					if (intFrameType == BREAK && DebugLog)
						WriteDebugLog(LOGDEBUG,
									  "[ARDOPprotocol.ProcessRcvdARQFrame] BREAK received in ARQState %s Processed as implied ACK",
									  ARQSubStates[ARQState]);
					...
					sprintf(HostCmd, "CONNECTED %s %d", strRemoteCallsign, intSessionBW);
					QueueCommandToHost(HostCmd);
					sprintf(HostCmd, "STATUS ARQ CONNECTION ESTABLISHED WITH %s, SESSION BW = %d HZ", strRemoteCallsign,
							intSessionBW);
					QueueCommandToHost(HostCmd);
					...
					if (intFrameType == BREAK && bytDataToSendLength == 0) {
						...
						WriteDebugLog(LOGDEBUG,
									  "[ARDOPprotocol.ProcessRcvdARQFrame] implied ACK, no data to send so action BREAK, send ACK");
				...
				{
					WriteDebugLog(LOGDEBUG,
								  "[ARDOPprotocol.ProcessRcvdARQFrame] ConRejBusy received in ARQState %s. Going to Protocol State DISC",
								  ARQSubStates[ARQState]);
			...
			if (ARQState == ISSData) {
				...
				if (intFrameType >= 0xE0)    // if ACK
				{
					...
					if (blnLastFrameSentData) {
						...
						if (strFrameType[bytLastARQDataFrameSent][0] == 'O')        // OFDM
						{
							...
							ComputeQualityAvg(
									38 + 2 * (intFrameType - 0xE0)); // Average ACK quality to exponential averager.
				...
				if (intFrameType == BREAK) {
					if (!blnARQConnected) {
						...
						sprintf(HostCmd, "CONNECTED %s %d", strRemoteCallsign, intSessionBW);
						QueueCommandToHost(HostCmd);
						sprintf(HostCmd, "STATUS ARQ CONNECTION ESTABLISHED WITH %s, SESSION BW = %d HZ",
								strRemoteCallsign, intSessionBW);
						QueueCommandToHost(HostCmd);
					}
					...
					WriteDebugLog(LOGDEBUG,
								  "[ARDOPprotocol.ProcessRcvdARQFrame] BREAK Rcvd from ARQState ISSData, Go to ProtocolState IRS & substate IRSfromISS , send ACK");
				...
				if (intFrameType <= 0x1F)         // if NAK
				{
					if (blnLastFrameSentData) {
						...
						ComputeQualityAvg(
								38 + 2 * intFrameType);     // Average in NAK quality to exponential averager.
...
BOOL SendARQConnectRequest(char *strMycall, char *strTargetCall) {
	...
	bytSessionID = GenerateSessionID(strMycall,
									 strTargetCall);  // Now set bytSessionID to receive ConAck (note the calling staton is the first entry in GenerateSessionID)
	bytPendingSessionID = bytSessionID;

	WriteDebugLog(LOGINFO, "[SendARQConnectRequest] strMycall=%s  strTargetCall=%s bytPendingSessionID=%x", strMycall,
				  strTargetCall, bytPendingSessionID);
	blnPending = TRUE;
	blnARQConnected = FALSE;
...
void LogStats() {
	...
	Statsprintf("  Squelch= %d BusyDet= %d Mode Shift UPs= %d   Mode Shift DOWNs= %d  Link Turnovers= %d", Squelch, BusyDet, intShiftUPs, intShiftDNs, intLinkTurnovers);
	Statsprintf(" ");
